...
class Networking {
                    ...
                    completion: @escaping CompletionHandler)
    {
        ...
        NDebug.shout("Data Task", "GET Request")
        NDebug.shout("urlPath", url)
        NDebug.shout("queries", "\(queries ?? "")")
        NDebug.shout("headers", headers as Any)
    ...
    fileprivate func handleNetworkingResult(_ data: Data?,
        ...
        if let error = error {
            ...
            NDebug.shout(TAG, "Response: \(response)")
            
            let nData = NData(data: Observable.just(data))
            NDebug.shout("\(TAG) NData:", nData.jsonString)
